...
import { FeedDTO } from "../DAO/interfaces/FeedDAO";
...
export class StatusService extends Service {
    public async postStatus  (
        ...
        await this.validatedOperation<void>(authToken, async () => {
          ...
          const authorAlias: string = newStatus.user.alias
          const data: DataPage<string> = await this.followDAO.getPageOfFollowers(authorAlias, Infinity, undefined)
          const followers: string[] = data.values

          for (let i = 0; i < followers.length; i++) {
            const ownerAlias = followers[i]
            await this.feedDAO.createFeedItem(ownerAlias, authorAlias, newStatus)
          }
        })
      };
    ...
    public async loadMoreFeedItems (
        ...
        // return this.getFakeData(lastItem, pageSize)
        return await this.validatedOperation<[StatusDto[], boolean]>(authToken, async () => {
          return await this.getMoreFeedItems(userAlias, pageSize, lastItem)
        })
      };

      private async getMoreFeedItems(userAlias: string, pageSize: number, lastItem: StatusDto | null): Promise<[StatusDto[], boolean]> {
        const dataPage: DataPage<FeedDTO> = await this.feedDAO.getPageOfFeedItems(userAlias, pageSize, lastItem?.timestamp)
        const hasMore: boolean = dataPage.hasMorePages
        const items: FeedDTO[] = dataPage.values
        const status: StatusDto[] = []
        for (let i = 0; i < items.length; i++) {
          const item = items[i]
          const user: User | null = await this.userDAO.getUserByAlias(item.alias)
      ...
      private async getMoreStoryItems(userAlias: string, pageSize: number, lastItem: StatusDto | null): Promise<[StatusDto[], boolean]> {
        ...
        const hasMore: boolean = dataPage.hasMorePages
        const items: StoryDTO[] = dataPage.values
        const status: StatusDto[] = []
        for (let i = 0; i < items.length; i++) {
          const item = items[i]
