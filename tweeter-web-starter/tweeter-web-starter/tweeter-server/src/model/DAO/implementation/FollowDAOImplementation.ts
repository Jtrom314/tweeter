...
export class FollowDAOImplementation extends DAOImplementation implements FollowDAO {
    ...
    async followUser(baseUserAlias: string, userToFollowAlias: string): Promise<void> {
        return await this.doAWSOperation(async () => {
            const command = new PutCommand({
                TableName: this.followTable,
                Item: {
                    [this.followerHandleField]: baseUserAlias,
                    [this.followeeHandleField]: userToFollowAlias
                }
            })
    ...
    async unfollowUser(baseUserAlias: string, userToFollowAlias: string): Promise<void> {
        return await this.doAWSOperation(async () => {
    ...
    async getPageOfFollowers(followeeHandle: string, pageSize: number, lastFollowerHanldle: string | undefined): Promise<DataPage<string>> {
        return await this.doAWSOperation(async () => {
            const command = new QueryCommand({
                ...
                Limit: pageSize,
                IndexName: 'index',
                KeyConditionExpression: `#followeeHandle = :followeeVal`,
            ...
            response!.Items!.forEach((item) => {
                items.push(item[this.followerHandleField].S as string)
            })
    ...
    async getPageOfFollowees(followerHandle: string, pageSize: number, lastFolloweeHandle: string | undefined): Promise<DataPage<string>> {
        return await this.doAWSOperation(async () => {
            const command = new QueryCommand({
                TableName: this.followTable,
                Limit: pageSize,
                KeyConditionExpression: `#followerHandle = :followerVal`,
                ExpressionAttributeNames: {
            ...
            const items: string[] = []
            response!.Items!.forEach((item) => {
                items.push(item[this.followeeHandleField].S as string)
            })
    
            return new DataPage<string>(items, hasMorePages)
        }, "Get page of followees")
    }
}